...
DATASET_DEFAULTS = {
    ...
    "amazon_electronics_photo": {
        "dataset": "amazon_electronics_photo",
        "data_root": r"../data",
        "extracted_root": None,
        "save_dir": None,
        "seed": 1,
    ...
    "amazon_electronics_computers": {
        "dataset": "amazon_electronics_computers",
        "data_root": r"../data",
        "extracted_root": None,
        "save_dir": None,
        "seed": 12,
...
def build_args():
    ...
    # ---------------- 数据集与路径配置 ----------------
    p.add_argument("--dataset", type=str, default="cora", help="数据集名称 (cora, citeseer, pubmed, acm, chameleon, bat, amap, amazon_electronics_photo, amazon_electronics_computers, texas, wiki)")
    ...
    p.add_argument("--update_weights", type=int, default=1, help="是否根据视图同质性动态更新融合权重 (1:是, 0:否)")
    ...
    p.add_argument("--dry_run", type=int, default=0, help="仅解析参数并打印后退出 (1:是, 0:否)")
    ...
    p.add_argument("--p_low_deg", type=float, default=0, help="低度边剪枝比例 (去除噪声边)")
    ...
    p.add_argument("--sample_loss_impl", type=str, default="v1", help="Sample-level 实现版本: v2(当前) / v1(同Model1)")
    ...
    # ---------------- SwAV参数 ----------------
    p.add_argument("--swav_variant", type=str, default="decor", help="SwAV版本: approx / classic / decor")
    ...
    # ---------------- 温度系数 ----------------
    p.add_argument("--tau_sample", type=float, default=0.3, help="Sample-level 对比损失的温度系数")
    ...
    if isinstance(defaults, dict) and defaults:
        provided = _provided_dests(p, sys.argv)
...
def main():
    args = build_args()
    if int(getattr(args, "dry_run", 0)) == 1:
        print(json.dumps(vars(args), ensure_ascii=False, sort_keys=True, indent=2))
        return
    set_seed(args.seed)
    ...
    labels, adj, features, adj_label, feature_label = load_dataset(
        ...
        amazon_computers_small_rebuild=bool(args.amazon_computers_small_rebuild),
        amazon_computers_use_small=bool(args.amazon_computers_use_small),
    )
